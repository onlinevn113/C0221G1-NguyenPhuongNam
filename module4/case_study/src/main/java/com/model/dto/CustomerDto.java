...
public class CustomerDto implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (customerDto.code.matches("")) {
            errors.rejectValue("code", "code.valid", "Please input ");
        } else if (!customerDto.code.matches("^KH-\\d{4}$")) {
        ...
        if (customerDto.birthday.matches("")) {
            errors.rejectValue("birthday", "birthday.valid", "Please input");
        } else if (!customerDto.birthday.matches(regex.DATE)) {
            errors.rejectValue("birthday", "birthday.valid", "FU");
        } else {
            int age = Period.between(LocalDate.parse(customerDto.birthday), LocalDate.now()).getYears();
            if (age < 18) {
